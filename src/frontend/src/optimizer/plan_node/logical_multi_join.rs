...
impl LogicalMultiJoin {
    ...
    ///    components. Repeat the procedure in 2. with the largest connected components down to the
    ///    smallest.
    /// 2. For each connected component, add joins to the chain, prioritizing adding those joins to
    ///    the bottom of the chain if their join conditions have: a. eq joins between primary keys
    ///    on both sides b. eq joins with primary keys on one side c. more equijoin conditions in
    ///    that order. This forms our selectivity heuristic.
    ...
    /// 4. Finally, we will emit, above the left-deep join tree: a. a filter with the non eq
    ///    conditions b. a projection which reorders the output column ordering to agree with the
